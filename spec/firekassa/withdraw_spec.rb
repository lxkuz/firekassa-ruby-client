...
RSpec.describe Firekassa::Withdraw do
  ...
  describe "#show" do
    subject(:result) { withdraw.show(Withdraw_id) }

    context "when transaction ID is valid", vcr: "withdraw/show" do
      let(:withdraw_id) { "507122561" }
    ...
    context "when transaction not found", vcr: "withdraw/show_not_found_error" do
      let(:withdraw_id) { "9999299990" }
      ...
      it "returns Withdraw not found error", aggregate_failures: true do
        expect { result }.to raise_error do |error|
          expect(error.data).to eql(error_data)
        end
      end
